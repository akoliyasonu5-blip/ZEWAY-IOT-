...
## GT800 configuration from the Kingwo MT100 manual

The **Kingwo MT100(User Manual)_2022.pdf**, section 4.2.3, allows GT800 on primary IP using `PROTOCOL,3,1#`, and primary server `IP,<host>,<port>,1#` for TCP (`0` is UDP). `PROTOCOL#` queries the active mode. Its `*11*1#` queries the center number, `*11*2#` queries longitude/latitude, and `*11*3#` queries device status. Factory reset is `*22*1#` and reboot is `*22*4#`; do not use either for testing. The manual does not state GT800 port 5095, `*11*4#`, `*11*5#ip,port#`, or GT800 location packet layout. These commands may belong to another device; do not apply them to the Kingwo tracker without confirming the actual unit. Our current raw listener parses JT/T 808, not GT800. A GT800 decoder requires the exact wire packet specification or a captured sample and its official field mapping.
